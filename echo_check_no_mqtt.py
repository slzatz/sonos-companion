'''
This is a python 2.7 program that may be run  on a raspberry pi
Requires three scripts:
echo_flask_sonos.py
echo_check_no_mqtt.py (this one)
sonos_echo_app.py
...
from config import user_id, location
...
COMMON_ACTIONS = {'pause':'pause', 'resume':'play', 'skip':'next', 'previous':'previous'}

def sonos_action(**task):
