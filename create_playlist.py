...
playlist_name = input("What do you want to call the playlist that will be uploaded to s3 [Note: should be in title case]? ")
playlist=[]
...
try:
    while 1:
        ...
        result = solr.query(collection, {'q':s, 'rows':5, 'fields':['score', 'title', 'artist', 'album'], 'sort':'score desc'}) 
        tracks = result.docs
        count = result.get_results_count()
        if count==0:
...
if res.lower().startswith('y'):
    ...
    response = s3.put_object(Bucket='sonos-scrobble', Key='playlists/'+playlist_name, Body=playlist)
