...
solr = SolrClient('http://54.173.234.69:8983/solr')
...
while 1:
    ...
    print("{} - checking sqs queue: {}; sonos master speaker: {}".format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), queue_name, master.player_name))
    ...
    if r:
        ...
        if action == 'deborah':
            ...
            result = solr.query(collection, {'q':task['trackinfo'], 'rows':1}) #..'rows':25 ...}, queryOptions='{"fields":["title", "artist"]}')

            if result.get_results_count():
                track = result.data['response']['docs'][0]
                try:
                    print('artist: ' + track.get('artist', ['No artist'])[0])
                    print('album: ' + track.get('album', ['No album'])[0])
                    print('song: ' + track.get('title', ['No title'])[0])
                except Exception as e:
                    print("Unicode error")
                uri = track.get('uri', [''])[0]
                print('uri: ' + uri)
            ...
            result = solr.query(collection, {'q':task['album']}) #..'rows':25 ...}, queryOptions='{"fields":["title", "artist"]}')

            if result.get_results_count():
                master.stop()
                master.clear_queue()
                for track in result.data['response']['docs']:
                    try:
                        print('artist: ' + track.get('artist', ['No artist'])[0])
                        print('album: ' + track.get('album', ['No album'])[0])
                        print('song: ' + track.get('title', ['No title'])[0])
            ...
            result = solr.query(collection, {'q':task['artist'], 'rows':100}) #..'rows':25 ...}, queryOptions='{"fields":["title", "artist"]}')
