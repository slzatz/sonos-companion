...
def get_release_date(artist, album):
    ...
    try:
        result = musicbrainzngs.search_releases(artist=artist, release=album, limit=3)  #for some reason 5 was pulling up some funky dates
    except:
        return "No date Exception"
    
    release_list = result['release-list']
             
    dates = [d['date'][0:4] for d in release_list if 'date' in d]
