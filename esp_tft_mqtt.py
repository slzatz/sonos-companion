...
def facts():
    ...
    shuffle(titles)
    titles = titles[:5]
    titles = [re.sub(r'\b\d+\.?\d+M?k?\b', '[  ]',title) for title in titles]
    titles = [re.sub(r'\b\d+M?k?\b', '[  ]',title) for title in titles] because single digit no decimal place are missed by the first re
