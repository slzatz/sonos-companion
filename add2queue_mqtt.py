'''
Add tracks to the Sonos queue and optionally create a playlist
...
Note this script will use that queue that echo_check.py or echo_check_mqtt.py has selected
'''
...
if res.lower()=='a':
    ...
    mqtt_publish.single('sonos/'+location, json.dumps({'action':'get sonos queue'}), hostname=ec_uri[7:], retain=False, port=1883, keepalive=60)
    sleep(2)
...
mqtt_publish.single('sonos/'+location, json.dumps({'action':action, 'uris':uris}), hostname=ec_uri[7:], retain=False, port=1883, keepalive=60)
if action == 'add':
    mqtt_publish.single('sonos/'+location, json.dumps({'action':'resume'}), hostname=ec_uri[7:], retain=False, port=1883, keepalive=60)
print('\n')
