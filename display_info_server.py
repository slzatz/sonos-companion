...
and then sends that information in an mqtt message with topic "display_info" 
...
Below are the styles currently recognized by of retrieve_unsplash_images_N:

fonts.addStyle("header", ofxFontStash2::Style("roboBlack", 36, ofColor::white));
...
fonts.addStyle("bodyWhite", ofxFontStash2::Style("robo", 18, ofColor::white));
...
import twitter
from config import tide_key, news_key, aws_mqtt_uri as aws_host, slz_twitter_oauth_token, slz_twitter_oauth_token_secret, slz_twitter_CONSUMER_KEY, slz_twitter_CONSUMER_SECRET, intrinio_username, intrinio_password, accuweather_apikey #exch_name, exch_pw, email
from lmdb_p import * 
import html
...
publish = partial(mqtt_publish.single, 'display_info', hostname=aws_host, retain=False, port=1883, keepalive=60)
...
def news():
    ...
    z = r.json()
    articles = [html.escape(x['title']) for x in z['articles'][:5]]
    print(datetime.datetime.now())
...
def weather_(pos):
    '''This was for ibm weather/weatherunderground and they shut it down'''
    #pos = 0
    ...
    try:
        ...
        data = {"header":"Weather", "text":text, "pos":pos, "dest":(1000,825)}
    except Exception as e:
        data = {"header":"Weather", "text":["Exception: {}".format(e)], "pos":pos, "dest":(1000,825)}
        
    publish(payload=json.dumps(data))

def weather(pos):
    '''Probably temporary since I don't want to pay $25/months to accuweather
       Adafruit uses openweathermap.org - need to check that out'''

    try:
        r = requests.get(f"http://dataservice.accuweather.com/forecasts/v1/daily/1day/2567_PC?apikey={accuweather_apikey}&details=true")
        z = r.json()
    except Exception as e:
        print("Exception in weather", e)
        return
    
    if not 'DailyForecasts' in z:
        print("'DailyForecasts' not in result of accuweather request")
        return
    try:
        day_forecast = z["DailyForecasts"][0]["Day"]["LongPhrase"]
        night_forecast = z["DailyForecasts"][0]["Night"]["LongPhrase"]
        high_temp = z["DailyForecasts"][0]["Temperature"]["Maximum"]["Value"]
        low_temp = z["DailyForecasts"][0]["Temperature"]["Minimum"]["Value"]

        text = [f"<bodyWhite>The day forecast is {day_forecast}</bodyWhite>", 
                f"<bodyWhite>The night forecast is {night_forecast}</bodyWhite>",
                f"<bodyWhite>The temperature will range between {low_temp} and {high_temp}</bodyWhite>"]

        print(f"accuweather successful {datetime.datetime.now()}")
        print(repr(text).encode('ascii', 'ignore'))
        data = {"header":"AccuWeather", "text":text, "pos":pos, "dest":(1000,825)}
    except Exception as e:
        data = {"header":"AccuWeather", "text":["Exception: {}".format(e)], "pos":pos, "dest":(1000,825)}
...
def stock_quote(pos):
    ...
    try:
        ...
        tag = "bodyGreen" if info[1]['value'] > 0 else "bodyRed"
        info[1]['value'] = f"<{tag}>{info[1]['value']}</{tag}>"
        info[2]['value'] = f"<{tag}>{100*info[2]['value']:.2f}</{tag}>"
        info[3]['value'] = format(int(info[3]['value']), ',d') #volume
        info[4]['value'] = "<bodyWhite>" + info[4]['value'].split('T')[1].split('+')[0] + "</bodyWhite>" #last timestamp
    except Exception as e:
        ...
        for x in info:
            ...
            label = f"<bodyWhite>{x['item'].replace('_', ' ').title()}</bodyWhite>"
...
def todo(pos):
    ...
    titles = [f"<bodyBold>{task.title}</bodyBold>" if task.star else f"<bodyWhite>{task.title}</bodyWhite>" for task in tasks]
    #shuffle(titles)
    print(datetime.datetime.now())
    print(repr(titles).encode('ascii', 'ignore'))

    data = {"header":"Things you better do", "text":titles, "pos":pos, "dest":(1075,10)} #expects a list
    publish(payload=json.dumps(data))
...
def google_calendar(pos):
    ...
    for event in events:
        ...
        if start_dt.hour == 0:
            ...
            item = f"<bodyRed>{month} {str(start_dt.day)} </bodyRed><bodyWhite>{summary}</bodyWhite>"
        else:
            #item = "{{red}}{} {}: {{green}}{} - {}: {{}}{}".format(month, str(start_dt.day),
            item = (
              f"<bodyRed>{month} {str(start_dt.day)}: </bodyRed><bodyGreen> "
              f"{start_dt.strftime('%-H:%M')} - {end_dt.strftime('%-H:%M')}: "
              f"</bodyGreen><bodyWhite>{summary}</bodyWhite>")
    ...
    data = {"header":"Google Calendar", "text":text, "pos":pos, "dest":(-600, 50)} #expects a list
    publish(payload=json.dumps(data))
        ...
        for msg in messages:
            ...
            for header in message['payload']['headers']:
                if header['name']=='Subject':
                    ...
                    item = ['<bodyBold>' + header['value'] + '</bodyBold>', html.escape(message['snippet'])+' ...', '']
                    text.extend(item)
                    break

    if not text:
        text = ['#There are no unread messages in Micropython or Deborah']
    data = {"header":"Gmail", "text":text, "pos":pos, "font size":12, "dest":(-600,-800), "bullets":False} #expects a list
    publish(payload=json.dumps(data))
...
def poems(pos): 
    ...
    #text = [f"<bodyBold>{title}</bodyBold>"]
    data = {}
    ...
    print("This is the poem note")
    print(repr(note))
    note[0] = f"<bodyItalic>{note[0]}"
    note[len(note) - 1] = f"{note[len(note) - 1]}</bodyItalic>"

    text.extend(note)
    print(repr(text))

    data.update({"header":"Poems ...", "text":text, "pos":pos, "bullets":False, "font size":14, "dest":(450,100)}) #text expects a list
    publish(payload=json.dumps(data))
...
def context(pos): 
    #pos = 16
    task = session.query(Task).join(Context).filter(Context.title==context_name, Task.star==True, Task.completed==None, Task.deleted==False, Task.modified > (datetime.datetime.now()-datetime.timedelta(days=100))).order_by(func.random()).first()
    #task = session.query(Task).join(Context).filter(Context.title==context, Task.star==True, Task.completed==None, Task.deleted==False).order_by(func.random()).first()
    if not task:
        return
    title = "#[{}] {}".format(task.context.title.capitalize(), task.title)
    note = task.note[:750] if task.note else '' # would be nice to truncate on a word
    ...
    text.extend(note.split("\n"))
    data = {"header":context_name.capitalize() + "...",
            "text":text[:10],
            "pos":pos, "bullets":False,
            "dest":(15,380),
            "font size":12} #text expects a list
    publish(payload=json.dumps(data))
